pub fn e_box(val: u8) -> u8 {
    ...
    let e_result = e_box[xy_idx.0][xy_idx.1];
    e_result
}

pub fn l_box(val: u8) -> u8 {
    let xy_idx = box_xy_idx(val);
    ...
    l_box[xy_idx.0][xy_idx.1]
}

pub fn inv_s_box(val: u8) -> u8 {
    
    let xy_idx = box_xy_idx(val);
    let inv_s_box: [[u8; 16];16] = 
    [
...
fn box_xy_idx(val: u8) -> (usize, usize) {
    (((val & 0xf0) >> 4) as usize, (val & 0x0f) as usize)
